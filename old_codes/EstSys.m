...
function [xh,yh,ph,pp,xp,yp,wpp,wxp]=EstSys(t,x0,p0,u,y,f,g,pf)
    ...
    if isempty(u)
     y0=g(t(1),p0,x0,[]);           % initial observation
    else  
    y0=g(t(1),p0,x0,u(:,1));    % initial observation    
    end
    ...
    for j=1:nx
        for i=1:np                          % simulate initial particles
            xp(j,1,i)=x0(j)+gen_state_noise(pf.noise.sigma_x(j));
        end
    end
    ...
    for k = 2:T
        ...
        for i=1:np
            ...
            if mean((p_obs_noise (y(:,k)-ynew,0,pf.noise.sigma_y)+1e-300))>mean((p_obs_noise (y(:,k)-yold,0,pf.noise.sigma_y)+1e-300))
            wpp(k,i) = wpk(i) *mean((p_obs_noise (y(:,k)-ynew,0,pf.noise.sigma_y)+1e-300));  %%%% mean of p's yp given xp
            pp(:,k,i)=pProposed;
           else
            wpp(k,i) = wpk(i) *mean((p_obs_noise (y(:,k)-yold,0,pf.noise.sigma_y)+1e-300));  %%%% mean of p's yp given xp
           pp(:,k,i)=pp(:,k-1,i);
           end
             %   
...
%         idx = randsample(1:np, np, with_replacement, wk(idx(idx/2+1:end)));
        ...
        for i=1:np
            w=zeros(nx,1);
            for j=1:nx
                w(j)=gen_state_noise(pf.noise.sigma_x(j));
            end
            if isempty(u)
            ...
            wxp(k,i) = wxk(i) *mean((p_obs_noise (y(:,k)-yp(:,k,i),0,pf.noise.sigma_y)+1e-300));  %%%% mean of p's yp given xp
